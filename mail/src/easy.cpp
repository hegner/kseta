/* Example program that introduces to task based parallelism
 g++ easy.cpp -o easy -std=c++14 -fgnu-tm -pthread
*/
#include <thread>
#include <vector>
#include <iostream>
...
//--------
   void push(const T& item){ while(!try_push(item));};
//---------
...
//---------
   void pop(T& item){ while(!try_pop(item));};
//---------
   bool try_pop (T& item) {
      bool success = false;
      if (m_currentSize > 0){
...
//--------
   void dump(){ // Non thread safe: here for debugging
      for (int i=0; i<m_currentSize; ++i){
         std::cout << "Item " << i << " " << m_items[i] << std::endl;
      }
   }
...
void tsPrint(const std::string& msg, std::thread::id thrId){
   auto printLambda = [thrId](std::string msgCopy){ std::cout << "[" << thrId << "] " << msgCopy << std::endl;};
...
void doWork(float deltaTf){
      Duration deltaT(deltaTf);
      Duration elapsed_seconds;
      TimePoint start,end;
      start = std::chrono::system_clock::now();
      while (elapsed_seconds<deltaT){
...
public:
   ...
   enum class State : char {kStart, kFolded, kStuffed, kSealed, kAddressed, kStamped, kMailed};
...
void Mail(mailItem& item){
   ...
   tsPrint("Mail item " + std::to_string(item.getId()) + " was sent.",std::this_thread::get_id());
...
void Stamp(mailItem& item){
   ...
   tsPrint("Mail item " + std::to_string(item.getId()) + " was stamped.",std::this_thread::get_id());
...
void Address(mailItem& item){
   ...
   tsPrint("Mail item " + std::to_string(item.getId()) + " was addressed.",std::this_thread::get_id());
   gActionsQueue->push(work);
}
...
void Seal(mailItem& item){
   ...
   tsPrint("Mail item " + std::to_string(item.getId()) + " was sealed.",std::this_thread::get_id());
   gActionsQueue->push(work);
}
...
void Stuff(mailItem& item){
   ...
   tsPrint("Mail item " + std::to_string(item.getId()) + " was stuffed.",std::this_thread::get_id());
   gActionsQueue->push(work);
}
...
void Fold(mailItem& item){
   ...
   tsPrint("Mail item " + std::to_string(item.getId()) + " was folded.",std::this_thread::get_id());
...
// It can sleep.
void pullWork(TsActionPtrQueue* workQueue,
...
int main(int argc, char** argv){
   ...
   std::cout << "Starting with " << nItems << " items and "
                  << nThreads << " threads\n";
   //-------------------------------------------------------
   ...
   std::thread msgSvcThr(pullWork,gMsgQueue,[&workEnded]{return !workEnded;},true);
   ...
   std::vector<mailItem> mailItems; mailItems.reserve(nItems);
